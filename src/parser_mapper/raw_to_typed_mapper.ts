  ...
  if (isProgram(ast)) {
    ...
    ast.top_declarations.forEach((node: any) => {
      if (isFunctionNode(node)) {
        ...
        throw new Error("Top declaration must be either a function node or a declaration");
      }
    });
    ...
    ast.ids.forEach((node: any) => {
      if (isIdentifier(node)) {
        ...
        throw new Error("Left hand side of assignment must be an identifier or an index access");
      }
    });
    ast.vals.forEach(verifyNode)
    ...
    ast.fields.forEach((node: any) => {
      if (!isStructFieldInstantiation(node)) {
        throw new Error("Struct field instantiation must be a struct field instantiation")
      }
    });
    ...
    if (ast.cases.some((node: any) => !(isSelectCase(node) || isDefaultCase(node)))) {
...
export function convert(ast: any): GoNode {
  if (isProgram(ast)) {
    return new Program(ast.top_declarations.map(convert) as (Declaration | FunctionNode | TypeDeclaration)[]);
  } else if (isDeclaration(ast)) {
    return new Declaration(ast.declaration_type, ast.ids.map(convert) as Identifier[], ast.vals.map(convert) as GoNode[]);
  } else if (isIdentifier(ast)) {
    return new Identifier(ast.name, ast.val_type === null ? null : convert(ast.val_type) as Type);
  } else if (isLiteral(ast)) {
    ...
    return new AssignmentStatement(ast.ids.map(convert) as (Identifier | IndexAccess)[], ast.vals.map(convert));
  } else if (isIfStatement(ast)) {
    return new IfStatement(convert(ast.cond), ast.cons.map(convert), ast.alt === null ? null : ast.alt.map(convert), ast.short === null ? null : convert(ast.short));
  } else if (isForStatement(ast)) {
    return new ForStatement(ast.init === null ? null : convert(ast.init), ast.cond === null ? null : convert(ast.cond), ast.post === null ? null : convert(ast.post), ast.body.map(convert));
  } else if (isGoStatement(ast)) {
    return new GoStatement(convert(ast.app) as Application);
  } else if (isFunctionNode(ast)) {
    return new FunctionNode(ast.name === null ? null : convert(ast.name) as Identifier, ast.formals.map(convert) as Identifier[], ast.retType === null ? null : convert(ast.retType) as Type, ast.body.map(convert));
  } else if (isIndexAccess(ast)) {
    ...
    return new StructAccess(convert(ast.accessed), convert(ast.field) as Identifier);
  } else if (isStructLiteral(ast)) {
    return new StructLiteral(convert(ast.val_type) as Type, ast.fields.map(convert) as StructFieldInstantiation[]);
  } else if (isStructFieldInstantiation(ast)) {
    return new StructFieldInstantiation(convert(ast.field) as Identifier, convert(ast.expr));
  } else if (isStructElement(ast)) {
    ...
    return new FunctionType(ast.formal_values.map(convert) as Type[], convert(ast.return_value) as Type);
  } else if (isChanType(ast)) {
    return new ChanType(ast.send_receive_type, convert(ast.chan_value_type) as Type);
  } else if (isArrayType(ast)) {
    ...
    return new TypeDeclaration(convert(ast.name) as CustomType, convert(ast.dec_type) as Type);
  } else if (isSelectStatement(ast)) {
    return new SelectStatement(ast.cases.map(convert) as (SelectCase | DefaultCase)[]);
  } else if (isSelectCase(ast)) {
